...
export default function UserProfile() {
    ...
    const { session, setSession, fetchSessionAndUserStatus } = useStore();
    ...
    const signout = () => {
        startSignOut(async () => {
            ...
            await fetchSessionAndUserStatus();
    ...
    useEffect(() => {
        if (!session) {
            router.push('/'); // Redirect to home page or login page if session is null
        }
    }, [session, router]);

    if (!userData || !session) {
        return null; // Don't render anything if data or session is not available
    }
                    ...
                    >
                        ...
                        <div className="space-y-5 w-full flex-1">
                            ...
                            <div className="flex gap-2 w-full pr-3 sm:pr-0">
                                <Button
                                    className="w-1/2 h-9 rounded-xl flex items-center justify-center gap-2 text-foreground text-sm"
                                    variant="outline"
                                    onClick={() => {
                                        document
